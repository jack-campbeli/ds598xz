...
class AgentPolicy(AgentWithModel):
    ...
    def get_reward(self, game, is_game_finished, is_new_turn, is_game_error):
        ...
        # Adjust rewards for coal and uranium collection, especially in the late game
        ...
        if Constants.RESOURCE_TYPES.WOOD in game.map.resources_by_type:
            num_of_wood_sources = len(game.map.resources_by_type[Constants.RESOURCE_TYPES.WOOD])
            # remove penality for early game
            if not late_game_phase:
                rewards["rew/d_wood_alive"] = (self.num_of_wood_sources_last - num_of_wood_sources) * 0.4
            self.num_of_wood_sources_last = num_of_wood_sources
        ...
        rewards["rew/close_to_resources"] = self.calculate_cities_near_resources(game) * 0.2
         ...
         # Penalize cities running out of fuel more as the game progresses
        for city in game.cities.values():
            if city.team == self.team and city.fuel < city.get_light_upkeep() * (total_turns - current_turn):
                rewards["rew/cities_running_out_of_fuel"] = -0.5 * late_game_multiplier  # Increase penalty in the late game
